...
* *Reporting*: Con esta versión, ahora puedes generar y descargar informes para ayudarte a analizar tu paisaje. Los informes prediseñados resumen las recuperaciones tras fallos y las recuperaciones tras fallos, muestran los detalles de la replicación en todos los sitios y muestran los detalles del trabajo de los últimos siete días.
+
...
* *Prueba gratuita de 30 días*: Con esta versión, ahora puede inscribirse para una prueba gratuita de 30 días de recuperación de desastres de BlueXP . Anteriormente, las pruebas gratuitas eran de 90 días.
+
Consulte https://docs.netapp.com/us-en/bluexp-disaster-recovery/get-started/dr-licensing.html["Configurar la licencia"].

* *Deshabilitar y habilitar planes de replicación*: Una versión anterior incluía actualizaciones de la estructura de programación de prueba de conmutación por error, que era necesaria para soportar horarios diarios y semanales. Esta actualización requería deshabilitar y volver a activar todos los planes de replicación existentes para poder utilizar los nuevos programas de prueba de conmutación por error diarios y semanales. Este es un requisito único.
+
Le contamos cómo:
...
* *Asignación de carpetas*: Cuando crea un plan de replicación y asigna recursos de computación, ahora puede asignar carpetas para que las VM se recuperen en una carpeta que especifique para el centro de datos, el clúster y el host.
+
...
* *Detalles de VM disponibles para failover, failback y failover de prueba*: Cuando ocurre un fallo y usted está iniciando un failover, realizando un failback o probando el failover, ahora puede ver detalles de las VM e identificar qué VM no se reiniciaron.
+
...
* *VM name aliasing*: Cuando creas un plan de replicación, ahora puedes agregar un prefijo y un sufijo a los nombres de VM en el área de recuperación ante desastres. Esto permite usar un nombre más descriptivo para las máquinas virtuales del plan.
+
Para obtener más información, consulte https://docs.netapp.com/us-en/bluexp-disaster-recovery/use/drplan-create.html["Cree un plan de replicación"] .

* * Limpiar instantáneas antiguas*: Con esta versión, puede eliminar cualquier instantánea que ya no sea necesaria más allá de su recuento de retención especificado. Las copias Snapshot pueden acumularse con el tiempo al reducir el número de retención de snapshots y ahora puede quitarlas para liberar espacio. Puede hacerlo en cualquier momento bajo demanda o cuando elimine un plan de replicación.
+
